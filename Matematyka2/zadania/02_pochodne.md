...
Na koniec stwórz **jeden rysunek** (w Matlabie `figure`), zawierający cześć wykresów (`subplot`) zorganizowanych w dwa wiersze i trzy kolumny. W pierwszej kolumnie ma się znaleźć jeden pod drugim analiza zbieżności całki funkcji i analiza zbieżności całki pochodnej funkcji. Druga kolumna ma przedstawiać wyjściową funkcję oraz jej pochodną dla dziedziny pięcioelementowej, a trzecia: dla dziedziny, w której uzyskano zbieżność. Styl linii powinien zawierać znaczniki ilustrujące położenie punktów. Nadaj wykresom odpowiednie tytuły informujące o liczbie elementów dziedziny. Zapisz końcowy rysunek do formatu `.png`. Oceń, czy obie całki zbiegają się w jednakowy sposób; zastanów się, z czego wynika różnica.
